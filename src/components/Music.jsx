        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {videos.map((video) => (
            <Link to={`/video/${video.id.videoId}`} key={video.id}>
              <li 
                className="bg-gray-800 rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300"
              >
                {video.snippet && video.snippet.thumbnails && video.snippet.thumbnails.high && video.snippet.thumbnails.high.url ? (
                  <img
