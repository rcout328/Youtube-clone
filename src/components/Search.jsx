...
function Main() {
  ...
  useEffect(() => {
    if (searchQuery.trim()) { // Check if the search query is not just empty spaces
      const url = `https://youtube-v31.p.rapidapi.com/search?q=${encodeURIComponent(searchQuery)}&part=snippet%2Cid&regionCode=US&maxResults=50&order=date`;
        ...
        {videos.length > 0 ? (
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {videos.map((video, index) => (
              <Link to={`/searchmain/${video.id.videoId}`} key={video.id.videoId || index}>
                <li className="bg-gray-800 rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                  ...
                  <div className="p-5">
                    <h2 className="text-xl font-semibold mb-2 hover:text-red-500 transition-colors duration-300">{video.snippet.title}</h2>
